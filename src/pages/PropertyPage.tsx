...
const PropertyPage = () => {
    ...
    return (
        <div className="bg-white min-h-screen text-navy-deep font-body">
            ...
            {/* 3. Service Office & Virtual Office Section */}
                ...
                <div className="container mx-auto px-6 relative z-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
                        ...
                        >
                            <div className="space-y-4">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-px bg-[#BC9C33]" />
                                    <span className="text-[#BC9C33] text-[10px] font-black uppercase tracking-[0.4em]">Premium Mobility</span>
                                </div>
                                <h2 className="text-5xl md:text-7xl font-display text-navy-deep leading-[1.1] tracking-tight">
                                    Service & Virtual <br />
                                    <span className="text-[#BC9C33]">Office Solutions</span>
                                </h2>
                            </div>

                            <p className="text-slate-500 text-lg md:text-xl font-light leading-relaxed max-w-xl">
                                Kami memiliki Service Office dan Virtual Office yang dapat disewa harian, mingguan, bulanan, bahkan tahunan. Solusi kerja fleksibel di lokasi prestisius.
                            </p>

                            <div className="flex flex-wrap gap-4 pt-4">
                                {["Daily", "Weekly", "Monthly", "Annual"].map((period) => (
                                    <span key={period} className="px-6 py-2 bg-slate-50 border border-slate-100 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-400">
                                        {period} Term
                                    </span>
                                ))}
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-6 md:gap-8">
                            {[
                                { name: "Listrik & AC", icon: "⚡" },
                                { name: "High-Speed Wifi", icon: "🌐" },
                                { name: "Print & Copy", icon: "🖨️" },
                                { name: "Shared Pantry", icon: "☕" },
                                { name: "Exclusive Lounge", icon: "🛋️" },
                                { name: "Musholla", icon: "🕌" },
                                ...
                                <motion.div
                                    ...
                                    className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/40 hover:-translate-y-1 transition-all group"
                                >
                                    ...
                                    <h4 className="text-[11px] font-black uppercase tracking-widest text-navy-deep group-hover:text-[#BC9C33] transition-colors">
                                        {facility.name}
                                    </h4>
                                </motion.div>
                            ))}
                                ...
                                <div className="w-full lg:w-1/2 relative group">
                                    <div className="relative aspect-[4/3] overflow-hidden rounded-card shadow-2xl">
                                        ...
                                        <div className="absolute bottom-6 left-6 z-20 bg-white/90 backdrop-blur px-6 py-3 rounded-full flex items-center gap-3 shadow-lg">
                                            <MapPin size={18} className="text-[#BC9C33]" />
                                            <span className="text-navy-deep text-xs font-bold uppercase tracking-wider">{project.location}</span>
                                        </div>
                                    </div>

                                    {/* Decorative Element */}
                                    <div className={`absolute -bottom-10 ${project.reverse ? '-left-10' : '-right-10'} w-2/3 h-2/3 border border-[#BC9C33]/30 rounded-card -z-10`}></div>
                                </div>

                                {/* Content Section */}
                                <div className="w-full lg:w-1/2">
                                    <div className="flex items-center gap-3 mb-6">
                                        <div className="h-px w-12 bg-[#BC9C33]"></div>
                                        <span className="text-[#BC9C33] font-bold uppercase tracking-widest text-xs">{project.subtitle}</span>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                                        {project.points.map((point, i) => (
                                            <div key={i} className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full bg-[#BC9C33]/10 flex items-center justify-center text-[#BC9C33]">
                                                    <Sparkles size={14} />
                                                </div>
                                    ...
                                    <button className="group flex items-center gap-3 text-navy-deep font-bold border-b-2 border-[#BC9C33] pb-1 hover:text-[#BC9C33] transition-colors">
