        ...
        return (
            <div className="bg-white min-h-screen flex items-center justify-center">
                <div className="text-center">
                    ...
                    <Link to="/news" className="text-[#BA9B32] font-bold uppercase tracking-widest text-xs">Return to Archive</Link>
                </div>
            </div>
        );
    }
                ...
                <div className="container mx-auto px-6 relative z-10">
                    <div className="max-w-4xl">
                        <motion.div
                            ...
                            className="flex items-center gap-4 text-white/60 text-xs font-bold uppercase tracking-[.4em] mb-8"
                        >
                            <Link to="/news" className="hover:text-[#BA9B32] transition-colors">Archive</Link>
                            <ChevronRight size={14} className="text-[#BA9B32]" />
                        ...
                        <motion.span
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="bg-[#BA9B32] text-white text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-full mb-8 inline-block shadow-2xl"
                        >
                            {article.category}
                        </motion.span>
                        ...
                        <motion.h1
                            {...fadeIn}
                            className="text-white text-3xl md:text-6xl font-display leading-[1.1] mb-8 md:mb-12"
                        >
                            {article.title}
                        </motion.h1>
                        ...
                        <motion.div
                            ...
                            className="flex flex-wrap items-center gap-8 text-[11px] font-bold uppercase tracking-[.3em] text-white/60"
                        >
                            <span className="flex items-center gap-2"><Calendar size={14} className="text-[#BA9B32]" /> {article.date}</span>
                            <span className="flex items-center gap-2"><User size={14} className="text-[#BA9B32]" /> {article.author}</span>
                            <span className="flex items-center gap-2"><Clock size={14} className="text-[#BA9B32]" /> 4 MIN READ</span>
                        </motion.div>
                    </div>
                </div>
            </header>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                        ...
                        <aside className="lg:col-span-1 flex lg:flex-col gap-6 items-center border-r border-slate-50 relative">
                            <div className="lg:sticky lg:top-32 flex lg:flex-col gap-4">
                                <button
                                    onClick={handleShare}
                                    className={`w-12 h-12 rounded-full border flex items-center justify-center transition-all ${showCopied ? 'bg-green-500 border-green-500 text-white' : 'border-slate-100 text-slate-400 hover:text-[#BA9B32] hover:border-[#BA9B32]'}`}
                                >
                                    {showCopied ? <Check size={18} /> : <Share2 size={18} />}
                                </button>
                                <button className="w-12 h-12 rounded-full border border-slate-100 flex items-center justify-center text-slate-400 hover:text-[#BA9B32] hover:border-[#BA9B32] transition-all">
                                    <Bookmark size={18} />
                                </button>
                            </div>
                        </aside>
                        ...
                        >
                            <p className="text-lg md:text-2xl font-light text-slate-500 italic border-l-4 border-[#BA9B32] pl-6 md:pl-8 mb-8 md:mb-16 leading-relaxed">
                                {article.excerpt}
                            </p>
                                    ...
                                    if (videoRegex.test(trimmedP) && trimmedP.length < 300) {
                                        return (
                                            <div key={idx} className="my-12 w-full aspect-video rounded-card overflow-hidden shadow-2xl bg-black flex items-center justify-center">
                                                ...
                                                >
                                                    <source src={trimmedP} type={`video/${trimmedP.split('.').pop()?.split('?')[0]}`} />
                            ...
                            {article.quote && (
                                <div className="my-20 p-12 bg-slate-50 rounded-card text-center relative overflow-hidden group">
                                    ...
                                    {article.quote_author && (
                                        <p className="mt-8 text-[#BA9B32] font-bold uppercase tracking-widest text-[10px]">{article.quote_author}</p>
                                    )}
                                </div>
                            )}
                            ...
                            <div className="mt-24 pt-12 border-t border-slate-100 flex items-center justify-between">
                                <Link
                                    to="/news"
                                    className="flex items-center gap-4 text-navy-deep font-bold uppercase tracking-widest text-xs hover:text-[#BA9B32] transition-colors"
                                >
                                    <ArrowLeft size={16} />
                                    Back to Archive
                                </Link>
