    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-navy-deep/20 backdrop-blur-sm">
            <motion.div
                ...
                className="bg-white rounded-[6px] w-full max-w-xl overflow-hidden shadow-2xl border border-slate-100 flex flex-col relative"
            >
                ...
                <div className="p-10 space-y-10 max-h-[65vh] overflow-y-auto custom-scrollbar text-left scroll-smooth relative z-10 bg-white">
                    ...
                    <div className="space-y-6 pt-10 border-t border-slate-50">
                        ...
                        <div className="bg-slate-50/50 p-8 rounded-[6px] border border-slate-50">
                ...
                <div className="p-10 bg-slate-50/50 border-t border-slate-50 flex justify-end relative z-20">
                    <button
                        onClick={onClose}
                        className="px-12 py-4 bg-navy-deep text-white rounded-[6px] font-black text-[10px] uppercase tracking-[0.2em] hover:bg-[#BC9C33] transition-all shadow-xl shadow-navy-deep/5 active:scale-95"
                    >
                        Terminal Close
                    </button>
                </div>
        ...
        <div className="space-y-12">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-4">
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-3 px-5 py-2.5 bg-white border border-slate-100 rounded-[6px] shadow-sm">
                ...
                <button
                    ...
                    className="group flex items-center justify-center gap-3 px-8 py-3.5 bg-white border border-slate-100 text-navy-deep rounded-[6px] font-black text-[10px] uppercase tracking-[0.2em] hover:bg-navy-deep hover:text-white transition-all shadow-sm active:scale-95"
                >
            ...
            <div className="bg-white p-8 rounded-[6px] border border-slate-100 shadow-sm flex flex-col lg:flex-row lg:items-center gap-8">
                <div className="flex-1 relative w-full group">
                    ...
                    <input
                        ...
                        className="w-full pl-14 pr-8 py-4 bg-slate-50/50 border border-slate-100 rounded-[6px] font-bold text-[11px] text-navy-deep placeholder:text-slate-300 focus:bg-white focus:border-slate-300 outline-none transition-all"
                    />
                </div>
                <div className="flex items-center gap-4 w-full lg:w-auto">
                    <div className="relative flex-1 lg:w-48">
                        <input
                            ...
                            className="w-full bg-slate-50/50 border border-slate-100 rounded-[6px] py-3.5 px-6 font-black text-[10px] uppercase tracking-widest text-navy-deep focus:bg-white focus:border-slate-300 outline-none transition-all"
                        />
                    </div>
                    <div className="w-4 h-0.5 bg-slate-100 rounded-[6px] shrink-0" />
                    <div className="relative flex-1 lg:w-48">
                        <input
                            type="date"
                            value={endDate}
            ...
            {/* Desktop Table View */}
            <div className="hidden md:block bg-white rounded-[6px] border border-slate-100 shadow-sm overflow-hidden min-h-[500px]">
                                    ...
                                    <td className="px-10 py-8 text-right">
                                        <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all duration-500 pr-2">
                                            <button
                                                ...
                                                className="w-11 h-11 flex items-center justify-center bg-white text-navy-deep rounded-[6px] border border-slate-100 hover:border-navy-deep transition-all duration-500 shadow-sm"
                                            >
                                                <Eye size={16} />
                                            </button>
                                            <button
                                                ...
                                                className="w-11 h-11 flex items-center justify-center bg-white text-red-400 hover:bg-red-50 hover:text-red-600 rounded-[6px] border border-slate-100 transition-all duration-500 shadow-sm"
                                            >
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
            ...
            <div className="md:hidden space-y-4">
                {currentItems.map((contact) => (
                    <motion.div
                        ...
                        className="bg-white p-6 rounded-[6px] border border-slate-100 space-y-6 text-left"
                    >
                        <div className="flex justify-between items-start gap-4">
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => setSelectedContact(contact)} className="w-10 h-10 flex items-center justify-center bg-white text-navy-deep rounded-[6px] border border-slate-100 shadow-sm active:scale-90 transition-transform"><Eye size={14} /></button>
                                <button onClick={() => setConfirmModal({ isOpen: true, id: contact.id, title: 'Confirm Deletion', message: `Purge message from ${contact.first_name}?` })} className="w-10 h-10 flex items-center justify-center bg-white text-red-400 rounded-[6px] border border-slate-100 shadow-sm active:scale-90 transition-transform"><Trash2 size={14} /></button>
                            </div>
                        </div>
                        <div className="bg-slate-50/50 p-5 rounded-[6px] border border-slate-50">
