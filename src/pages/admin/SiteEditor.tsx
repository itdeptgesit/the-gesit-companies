...
const SiteEditor = ({ initialTab = "hero" }: { initialTab?: string }) => {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-navy-deep rounded-[6px] p-10 text-white relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-12 opacity-10 group-hover:scale-110 transition-transform duration-700">
                    <Globe size={180} />
                </div>
                <div className="relative z-10 max-w-2xl">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-10 h-10 rounded-[6px] bg-amber-500 flex items-center justify-center shadow-lg shadow-amber-500/40">
            ...
            <div className="flex flex-wrap gap-2">
                {subTabs.map((tab) => (
                    <button
                        ...
                        className={`flex items-center gap-3 px-6 py-3.5 rounded-[6px] text-[11px] font-black uppercase tracking-widest transition-all ${activeSubTab === tab.id
            ...
            {/* Editor Content Area */}
            <div className="bg-white rounded-[6px] p-10 border border-slate-200 shadow-sm relative min-h-[400px]">
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button
                    onClick={fetchLogs}
                    className="p-2 bg-slate-100 rounded-[6px] hover:bg-slate-200 transition-colors text-slate-500"
                    title="Refresh Logs"
                >
                    <History size={16} />
                </button>
            ...
            <div className="space-y-4 relative before:absolute before:left-4 before:top-4 before:bottom-4 before:w-[2px] before:bg-slate-100">
                {isLoading ? (
                    Array(5).fill(0).map((_, i) => (
                        <div key={i} className="ml-12 h-16 bg-slate-50 rounded-[6px] animate-pulse" />
                    ))
                ) : error ? (
                    <div className="ml-12 p-8 border-2 border-red-100 bg-red-50 rounded-[6px] flex flex-col items-center justify-center text-red-400 gap-4">
                        <AlertCircle size={24} />
                    ...
                    logs.map((log) => (
                        <div key={log.id} className="ml-12 relative group">
                            <div className={`absolute -left-[41px] top-4 w-6 h-6 rounded-[6px] border-4 border-white shadow-sm flex items-center justify-center z-10 ${getActionColor(log.action).split(' ')[0]}`}>
                                <Activity size={10} className={getActionColor(log.action).split(' ')[1]} />
                            </div>

                            <div className="bg-slate-50 rounded-[6px] p-5 border border-slate-100 hover:shadow-md transition-shadow group-hover:bg-white">
                    ...
                    <div className="ml-12 p-8 border-2 border-dashed border-slate-200 rounded-[6px] flex flex-col items-center justify-center text-slate-300 gap-4">
...
const HeroManager = () => {
    ...
    const fetchSlides = async () => {
        ...
        if (error) console.error("Error fetching slides:", error);
            ...
            setSlides(data || []);
            // Set global text from the first slide in the sequence
            if (data && data.length > 0) {
                setGlobalTitle(data[0].title || "");
                setGlobalSubtitle(data[0].subtitle || "");
            }
        }
        setIsLoading(false);
    };
    ...
    const handleSaveGlobalText = async () => {
        setIsSaving(true);
        try {
            // Update ALL slides to use the same title and subtitle
            ...
            showToast("Banner text updated globally!", "success");
            await logActivity('UPDATE', 'Hero Text', `Updated global banner text: "${globalTitle}"`);
            fetchSlides();
        } catch (error: any) {
    ...
    const handleSave = async () => {
        ...
        const slideData = {
            title: globalTitle, // Always use global title
            subtitle: globalSubtitle, // Always use global subtitle
        ...
        if (error) {
            ...
            // Log Activity
            await logActivity(action, 'Hero Slides', `${action === 'CREATE' ? 'Created' : 'Updated'} slide`);
        ...
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-2">Manage visual banners and global text</p>
                </div>
                <button
                    onClick={() => { setEditingSlide({}); setIsFormOpen(true); }}
                    className="flex items-center gap-3 px-6 py-3 bg-amber-500 text-white rounded-[6px] text-[10px] font-black uppercase tracking-widest hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20 active:scale-95"
                >
            ...
            <div className="bg-slate-50 rounded-[6px] p-8 border border-slate-200">
                <div className="flex items-center gap-4 mb-6">
                    <div className="w-10 h-10 rounded-[6px] bg-white border border-slate-200 flex items-center justify-center text-amber-500 shadow-sm">
                        <Type size={20} />
                    </div>
                    <div>
                        <h4 className="text-sm font-black text-navy-deep tracking-tight uppercase">Banner Global Text</h4>
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">This text will stay the same for all slides</p>
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm focus:border-amber-500 outline-none transition-all font-display font-bold text-navy-deep"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm focus:border-amber-500 outline-none transition-all font-medium text-slate-600"
                            placeholder="Enter global sub-header..."
                        />
                    </div>
                </div>
                <div className="mt-6 flex justify-end">
                    <button
                        onClick={handleSaveGlobalText}
                        disabled={isSaving}
                        className="flex items-center gap-3 px-8 py-3 bg-navy-deep text-white rounded-[6px] text-[10px] font-black uppercase tracking-widest hover:bg-[#BC9C33] transition-all shadow-lg shadow-navy-deep/20"
                    >
                        {isSaving ? "Updating..." : <><Save size={16} /> Update All Text</>}
                    </button>
                </div>
            </div>
            ...
            {isFormOpen && (
                <motion.div
                    ...
                    className="p-8 bg-white rounded-[6px] border-2 border-slate-100 shadow-xl space-y-6 overflow-hidden"
                >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            <div className="bg-slate-50 p-6 rounded-[6px] border border-slate-100">
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-navy-deep mb-3">Slide Details</h4>
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm focus:border-amber-500 outline-none transition-all font-mono"
                                        />
                                    </div>
                                    <div className="p-4 bg-amber-50 rounded-[6px] border border-amber-100">
                                        <p className="text-[9px] text-amber-700 font-bold uppercase leading-relaxed">
                                            The global title <span className="text-[#BC9C33]">"{globalTitle}"</span> will be applied to this slide automatically.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative aspect-video rounded-[6px] overflow-hidden bg-slate-200 border border-slate-300 group shadow-inner">
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 mt-2">
                        ...
                        <button
                            onClick={handleSave}
                            disabled={isSaving || !editingSlide?.image_url}
                            className="flex items-center gap-3 px-8 py-3 bg-navy-deep text-white rounded-[6px] text-[10px] font-black uppercase tracking-widest hover:bg-[#BC9C33] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-navy-deep/20"
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {isLoading ? (
                    Array(2).fill(0).map((_, i) => (
                        <div key={i} className="h-64 bg-slate-50 rounded-[6px] animate-pulse border border-slate-100" />
                    ))
                ) : slides.length > 0 ? (
                    slides.map((slide) => (
                        <div key={slide.id} className="group bg-white border border-slate-100 rounded-[6px] overflow-hidden shadow-sm hover:shadow-xl hover:translate-y-[-4px] transition-all duration-500 relative">
                            <div className="relative aspect-video overflow-hidden">
                                ...
                                <div className="absolute top-4 right-4 flex gap-2 translate-y-[-10px] opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                                    <button
                                        ...
                                        className="w-9 h-9 bg-white/10 backdrop-blur-md rounded-[6px] flex items-center justify-center text-white hover:bg-white hover:text-navy-deep transition-all active:scale-90 border border-white/20"
                                    >
                                        <Type size={14} />
                                    </button>
                                    <button
                                        onClick={() => handleDelete(slide)}
                                        className="w-9 h-9 bg-white/10 backdrop-blur-md rounded-[6px] flex items-center justify-center text-white hover:bg-red-500 transition-all active:scale-90 border border-white/20"
                                    >
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                    ...
                    <div className="md:col-span-2 p-16 border-2 border-dashed border-slate-200 rounded-[6px] flex flex-col items-center justify-center text-slate-300 gap-6 bg-slate-50/50">
                        <div className="w-20 h-20 rounded-[6px] bg-slate-100 flex items-center justify-center">
...
const HomeConfigManager = () => {
    ...
    const handleSave = async () => {
        ...
        if (error) {
            showToast("Error saving segment: " + error.message, "error");
        } else {
            showToast("Business segment updated!", "success");
            // Log Activity
        ...
        <div className="space-y-8">
            ...
            {isFormOpen && (
                <motion.div
                    initial={{ opacity: 0, height: 0 }}
                    animate={{ opacity: 1, height: 'auto' }}
                    className="p-8 bg-slate-50 rounded-[6px] border border-slate-200 space-y-6 overflow-hidden"
                >
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm focus:border-amber-500 outline-none transition-all font-bold text-navy-deep"
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm focus:border-amber-500 outline-none transition-all h-24 resize-none font-medium text-slate-600"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm focus:border-amber-500 outline-none transition-all"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {isLoading ? (
                    Array(4).fill(0).map((_, i) => (
                        <div key={i} className="h-80 bg-slate-50 rounded-[6px] animate-pulse border border-slate-100" />
                    ))
                ) : segments.length > 0 ? (
                    segments.map((segment) => (
                        <div key={segment.id} className="group bg-white border border-slate-100 rounded-[6px] overflow-hidden shadow-sm hover:shadow-xl hover:translate-y-[-4px] transition-all duration-500 relative flex flex-col">
                            <div className="relative aspect-[4/3] overflow-hidden shrink-0">
                                ...
                                <div className="absolute top-4 right-4 flex gap-2 translate-y-[-10px] opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                                    <button
                                        ...
                                        className="w-8 h-8 bg-white/10 backdrop-blur-md rounded-[6px] flex items-center justify-center text-white hover:bg-white hover:text-navy-deep transition-all active:scale-90 border border-white/20"
                                    >
                                        <Type size={12} />
                                    </button>
                                </div>
                    ...
                    <div className="col-span-full p-16 border-2 border-dashed border-slate-200 rounded-[6px] flex flex-col items-center justify-center text-slate-300 gap-6 bg-slate-50/50">
                        <div className="w-20 h-20 rounded-[6px] bg-slate-100 flex items-center justify-center">
                        ...
                        <div className="text-center">
                            ...
                            <button
                                ...
                                className="px-6 py-2 bg-slate-200 text-slate-600 rounded-[6px] text-[10px] font-black uppercase tracking-widest hover:bg-navy-deep hover:text-white transition-all transform hover:scale-105 active:scale-95"
                            >
                                Initialize Defaults
                            </button>
                        </div>
...
const BusinessManager = () => {
    ...
    const SectionHeader = ({ title }: { title: string }) => (
        <div className="flex items-center gap-4 py-4 border-b border-slate-100 mb-6">
            <div className="p-2 bg-navy-deep/5 rounded-[6px] text-navy-deep"><FileText size={16} /></div>
            ...
            <div className="mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {Array.isArray(value) && value.map((img: string, idx: number) => (
                        <div key={idx} className="relative aspect-square rounded-[6px] overflow-hidden bg-slate-100 group border border-slate-200">
                            ...
                            <button
                                ...
                                className="absolute top-1 right-1 w-6 h-6 bg-red-500 text-white rounded-[6px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all scale-90 hover:scale-100"
                            >
                                <X size={12} />
                            </button>
                        </div>
                    ))}
                    <label className="aspect-square rounded-[6px] border-2 border-dashed border-slate-200 hover:border-navy-deep/50 hover:bg-navy-deep/5 transition-all flex flex-col items-center justify-center cursor-pointer text-slate-400 hover:text-navy-deep gap-2">
    ...
    const ProjectsEditor = ({ projects, onChange }: any) => {
        ...
        return (
            <div className="space-y-6">
                {editingIndex !== null ? (
                    <div className="bg-slate-50 p-6 rounded-[6px] border border-slate-200 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input value={projects[editingIndex].title} onChange={(e) => handleUpdate(editingIndex, 'title', e.target.value)} className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm font-bold text-navy-deep" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-2">Subtitle</label>
                                    <input value={projects[editingIndex].subtitle} onChange={(e) => handleUpdate(editingIndex, 'subtitle', e.target.value)} className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm font-medium" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-2">Location</label>
                                    <input value={projects[editingIndex].location} onChange={(e) => handleUpdate(editingIndex, 'location', e.target.value)} className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-2">Property Type</label>
                                    <input value={projects[editingIndex].propertyType} onChange={(e) => handleUpdate(editingIndex, 'propertyType', e.target.value)} className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm" />
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-2">Description</label>
                                    <textarea value={projects[editingIndex].desc} onChange={(e) => handleUpdate(editingIndex, 'desc', e.target.value)} className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm h-32 resize-none" />
                                </div>
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        className="w-full bg-white border border-slate-200 rounded-[6px] px-4 py-3 text-sm h-24 resize-none"
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {projects[editingIndex].images?.map((img: string, i: number) => (
                                    <div key={i} className="relative aspect-video rounded-[6px] overflow-hidden bg-slate-100 group border border-slate-200">
                                ...
                                <label className={`aspect-video rounded-[6px] border-2 border-dashed border-slate-200 hover:border-navy-deep/50 hover:bg-navy-deep/5 transition-all flex flex-col items-center justify-center cursor-pointer text-slate-400 hover:text-navy-deep gap-2 ${isUploading ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {projects.map((proj: any, idx: number) => (
                            <div key={idx} className="flex items-center gap-4 p-4 bg-white border border-slate-100 rounded-[6px] shadow-sm hover:shadow-md transition-all group">
                                <div className="w-16 h-16 rounded-[6px] bg-slate-100 overflow-hidden shrink-0">
                                ...
                                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => setEditingIndex(idx)} className="p-2 hover:bg-slate-100 rounded-[6px] text-slate-400 hover:text-navy-deep">
                                        <Type size={14} />
                                    </button>
                                    <button onClick={() => handleDelete(idx)} className="p-2 hover:bg-red-50 rounded-[6px] text-slate-400 hover:text-red-500">
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                            </div>
                        ))}
                        <button onClick={handleAdd} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-[6px] flex items-center justify-center text-slate-400 font-bold uppercase tracking-widest text-xs hover:border-navy-deep/20 hover:text-navy-deep hover:bg-navy-deep/5 transition-all">
        ...
        return (
            <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative aspect-video w-full rounded-[6px] overflow-hidden bg-slate-200 border border-slate-300 group shadow-inner">
    ...
    const renderEditor = () => {
        ...
        return (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="bg-white p-8 rounded-[6px] border border-slate-100 shadow-sm">
                ...
                <div className="sticky bottom-6 flex justify-end">
                    <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-3 px-8 py-4 bg-navy-deep text-white rounded-[6px] text-xs font-black uppercase tracking-widest hover:bg-[#BC9C33] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-xl shadow-navy-deep/20 hover:-translate-y-1 transform">
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Page Selector Tabs */}
            <div className="flex flex-wrap gap-2 p-1 bg-slate-100 rounded-[6px]">
                {[
                    ...
                    <button
                        ...
                        className={`flex-1 py-3 px-4 rounded-[6px] text-xs font-bold uppercase tracking-wider transition-all ${selectedPage === page.id
