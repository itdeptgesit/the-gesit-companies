...
export const CareerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const addJob = async (job: Omit<Career, 'id' | 'created_at'>) => {
        try {
            ...
            if (data) {
                ...
                await logActivity('CREATE', 'Careers', `Posted new job: "${job.title}"`);
            }
        } catch (err: any) {
    ...
    const updateJob = async (id: number, job: Partial<Career>) => {
        try {
            ...
            const { logActivity } = await import("../lib/tracking");
            await logActivity('UPDATE', 'Careers', `Updated job details for: "${jobs.find(j => j.id === id)?.title}"`);
        } catch (err: any) {
            setError(err.message);
            throw err;
        }
    ...
    const deleteJob = async (id: number) => {
        try {
            ...
            if (jobToDelete) {
                const { logActivity } = await import("../lib/tracking");
                await logActivity('DELETE', 'Careers', `Deleted job posting: "${jobToDelete.title}"`);
            }
        } catch (err: any) {
            setError(err.message);
            throw err;
    ...
    const deleteApplication = async (id: number, resumeUrl: string) => {
        try {
            ...
            if (appToDelete) {
                const { logActivity } = await import("../lib/tracking");
                await logActivity('DELETE', 'Careers', `Removed candidate application: "${appToDelete.full_name}"`);
            }
        } catch (err: any) {
            setError(err.message);
            throw err;
