...
export const NewsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const addNews = async (news: Omit<NewsItem, 'id'>) => {
        try {
            ...
            if (data) {
                ...
                await logActivity('CREATE', 'News', `Added new article: "${news.title}"`);
            }
        } catch (err: any) {
    ...
    const updateNews = async (id: number, news: Omit<NewsItem, 'id'>) => {
        try {
            ...
            const { logActivity } = await import("../lib/tracking");
            await logActivity('UPDATE', 'News', `Updated article: "${news.title}"`);
        } catch (err: any) {
    ...
    const deleteNews = async (id: number) => {
        try {
            ...
            if (itemToDelete) {
                const { logActivity } = await import("../lib/tracking");
                await logActivity('DELETE', 'News', `Deleted article: "${itemToDelete.title}"`);
            }
        } catch (err: any) {
